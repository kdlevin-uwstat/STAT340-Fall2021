...
Suppose that we have iid random variables $X_1,X_2,\dots$ drawn iid from a distribution with mean $\mu$ and variance $\sigma^2$.
...
__Note:__ we are estimating $\sigma^2$, not $\sigma$.
...
run_trial_adjusted <- function( nsamp ) {
  ...
  s2adj <- sigma2hat_adjusted( data );
  
  return( s2adj- 1) # Var = sd^2 = 1
}
...
estimate_bias_adjusted <- function( nsamp, NMC ) {
  ...
  for( i in 1:NMC ) {
    MCreps[i] <- run_trial_adjusted( nsamp );
  }
...
estimate_bias_adjusted( 20, 2000)
...
generate_data <- function( nsamp, lambda=1 ) {
  ...
  return( rpois(n=nsamp, lambda=lambda) );
}
...
estimate_pois_var <- function( data ) {
  ...
  return( estimate_pois_mean( data ) ); # the best programmer is the lazy one.
}
...
Implement a function `CLT_CI` that takes arguments `data` (the vector of data generated by `generate_data`) and `alpha` (a numeric between 0 and 1, i.e, a probability, which should default to $0.05$), and returns a cofidence interval (i.e., a vector of length 2 whose first entry is the left end of the interval and whose second entry is the right end of the interval).
...
CLT_CI <- function( data, alpha=0.05) {
  ...
  CI <- qnorm(c(alpha/2, 1-alpha/2), mean=Xbar, sd=sqrt(varhat));
...
simulation_CI <- function( data, alpha=0.05) {
  ...
  fakedata <- rpois(n=1000, lambda=Xbar );
  # Return a vector c(L, U) with L the left end and U the right ("upper") end of the CI. 
  CI <- quantile(fakedata, probs=c(alpha/2,1-alpha/2));
  return( as.vector(CI) );
}
...
contained <- function( myCI, trueparam ) {
  ...
  return( (trueparam > myCI[1]) & (trueparam < myCI[2]) );
}
...
* `CI_fn` is a *function* (in our code, this will be either `` or `simulation_CI`). Yes, functions can be arguments to other functions in R! Note that this argument will just be function. *Not a string.*
...
Assuming that the mule kicks data follows a Poisson distribution, produce an estimate for the rate parameter $\lambda$.
...
Using everything you know, construct a confidence interval for the rate parameter $\lambda$.
...
Code, plots, and/or explanation go here.
