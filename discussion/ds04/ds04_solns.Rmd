...
__Solution:__ the above is straight-forward plotting.
...
# We are just computing the sample mean directly below.
...
run_trial <- function( n, ptrue, Nrep ) {
  data <- rbinom(1, size=n, p=ptrue);
  phat <- data/n;
  for ( i in 1:Nrep) {
    replicates[i] <-resample_and_estimate( phat, n );
...
for( i in 1:Nexpt ) {
  ...
  ptrue_in_CI[i] <- run_trial( 200, 0.8, 1000 );
}
